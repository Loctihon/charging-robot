...
Ccs2DetectorNode::Ccs2DetectorNode(const rclcpp::NodeOptions &options)
    : Node("ccs2_detector_node", options), last_depth_(0.0f) {
  ...
  RCLCPP_INFO(this->get_logger(),
              "CCS2 detector — mode=%s  encoding=%s  clahe=%s  "
              "adaptive_morph=%s  aspect_filter=%s",
              use_color_filter_ ? "HSV_COLOR_FILTER" : "GAMMA_PIPELINE",
              image_encoding_.c_str(), use_clahe_ ? "ON" : "OFF",
  ...
  sync_ = std::make_shared<Synchronizer>(SyncPolicy(sync_queue_), rgb_sub_,
                                         depth_sub_);
  ...
  mask_pub_ =
      this->create_publisher<sensor_msgs::msg::Image>("~/mask_image", 10);
  raw_mask_pub_ =
      this->create_publisher<sensor_msgs::msg::Image>("~/raw_mask", 10);

  // Circle-detector debug image subscriber (visualization only)
  if constexpr (SHOW_PIPELINE) {
    circle_det_sub_ = this->create_subscription<sensor_msgs::msg::Image>(
        "/circle_detector/debug_image", rclcpp::SensorDataQoS(),
        [this](const sensor_msgs::msg::Image::ConstSharedPtr &msg) {
          try {
            cv::Mat frame = cv_bridge::toCvShare(msg, "bgr8")->image.clone();
            std::lock_guard<std::mutex> lk(circle_img_mutex_);
            last_circle_img_ = frame;
          } catch (const cv_bridge::Exception &e) {
            RCLCPP_WARN_ONCE(this->get_logger(),
                             "circle_detector image decode failed: %s",
                             e.what());
          }
        });
  }
...
void Ccs2DetectorNode::syncCallback(
  ...
  try {
    ...
    if (dc->image.type() == CV_32FC1) {
      ...
      RCLCPP_ERROR(this->get_logger(), "Unexpected depth type: %s",
  ...
  // Pipeline-capture struct (only filled when SHOW_PIPELINE == true)
  PipelineImages pipe;
  ...
  if (use_color_filter_) {
    ...
    if constexpr (SHOW_PIPELINE) {
      // HSV mode: no gamma pipeline — fill shared fields with grayscale
      cv::cvtColor(bgr, pipe.gray, cv::COLOR_BGR2GRAY);
      pipe.clahe = pipe.gray.clone();
      pipe.gamma = pipe.gray.clone();
    }
  } else {
    cv::Mat pre = preprocessRGB(bgr, SHOW_PIPELINE ? &pipe : nullptr);
    raw_mask = buildBinaryMaskGamma(pre);
    binarized_median = raw_mask.clone();
  }

  // Capture Otsu binary mask
  if constexpr (SHOW_PIPELINE) {
    pipe.otsu = raw_mask.clone();
  }
  ...
  cv::Rect bbox;
  det.detected =
      extractSocket(mask, socket_shape, depth32f, det, rot_rect, bbox);
  ...
  if (det.detected) {
    ...
    if (det.depth_m > 0.05f) {
      ...
      if (last_depth_ < 0.05f) {
        last_depth_ = det.depth_m;
      }
    }
  }
  ...
  // ── SHOW_PIPELINE: build detection tile and display dashboard ──────────────
  if constexpr (SHOW_PIPELINE) {
    // Build detection overlay — taps into variables already computed above,
    // mirrors the publishDebug logic without recomputing anything.
    cv::Mat det_viz = bgr.clone();
    if (det.detected && bbox.x >= 0 && bbox.y >= 0 &&
        bbox.x + bbox.width <= det_viz.cols &&
        bbox.y + bbox.height <= det_viz.rows) {
      cv::Mat ov = det_viz.clone();
      cv::rectangle(ov, bbox, cv::Scalar(147, 20, 255), cv::FILLED);
      cv::addWeighted(ov, 0.3, det_viz, 0.7, 0, det_viz);
      cv::rectangle(det_viz, bbox, cv::Scalar(0, 255, 0), 2);
      cv::drawMarker(det_viz,
                     cv::Point(static_cast<int>(det.centroid_x),
                               static_cast<int>(det.centroid_y)),
                     cv::Scalar(0, 255, 255), cv::MARKER_CROSS, 24, 2);
    } else {
      cv::putText(det_viz, "No socket", cv::Point(10, 40),
                  cv::FONT_HERSHEY_SIMPLEX, 0.8, cv::Scalar(0, 0, 255), 2);
    }
    pipe.detection = det_viz;

    // Snapshot circle-detector image (thread-safe read)
    cv::Mat circle_snap;
    {
      std::lock_guard<std::mutex> lk(circle_img_mutex_);
      circle_snap = last_circle_img_; // shallow copy of header + data ptr
    }

    showPipeline(bgr, pipe, circle_snap);
    cv::waitKey(1);
  }
}
...
cv::Mat Ccs2DetectorNode::preprocessRGB(const cv::Mat &bgr,
  ...
  cv::Mat clahe_out; // holds CLAHE result (or a reference to gray)
  if (use_clahe_) {
    auto clahe =
        cv::createCLAHE(clahe_clip_, cv::Size(clahe_grid_, clahe_grid_));
    clahe->apply(gray, gray);
    if constexpr (SHOW_PIPELINE) {
      clahe_out = gray.clone();
    }
  } else {
    if constexpr (SHOW_PIPELINE) {
      clahe_out = gray.clone();
  ...
  cv::Mat se = cv::getStructuringElement(
      cv::MORPH_ELLIPSE, cv::Size(2 * eff_radius + 1, 2 * eff_radius + 1));
  cv::Mat dilated;
  ...
  for (int i = 0; i < 256; ++i) {
    lut.at<uint8_t>(i) =
        cv::saturate_cast<uint8_t>(std::pow(i / 255.0, gamma_) * 255.0);
  }
  cv::Mat gamma_img;
  ...
  cv::Mat stretched;
  if (mx > mn) {
    gamma_img.convertTo(stretched, CV_8UC1, 255.0 / (mx - mn),
                        -mn * 255.0 / (mx - mn));
  } else {
  ...
  // ── Capture intermediates for SHOW_PIPELINE (zero cost when false) ─────────
  if constexpr (SHOW_PIPELINE) {
    if (pipe) {
      pipe->gray = gray.clone();
      pipe->clahe = clahe_out;
      pipe->gamma = gamma_img.clone();
    }
  }
...
cv::Mat Ccs2DetectorNode::buildBinaryMaskHSV(const cv::Mat &bgr) {
  ...
  cv::Mat mask;
  cv::inRange(hsv, cv::Scalar(hsv_h_low_, hsv_s_low_, hsv_v_low_),
              cv::Scalar(hsv_h_high_, hsv_s_high_, hsv_v_high_), mask);
...
cv::Mat Ccs2DetectorNode::applyMorphCleanup(const cv::Mat &raw_mask,
  ...
  cv::Mat se_disk = cv::getStructuringElement(cv::MORPH_ELLIPSE,
                                              cv::Size(2 * r + 1, 2 * r + 1));
  cv::Mat se_cross = cv::getStructuringElement(cv::MORPH_CROSS,
                                               cv::Size(2 * r + 1, 2 * r + 1));
  ...
  cv::Mat closed;
  cv::morphologyEx(dilated, closed, cv::MORPH_CLOSE, se_disk, cv::Point(-1, -1),
                   2);

  // Hole fill: flood background → invert → OR back
  cv::Mat filled = closed.clone();
  cv::Mat flood_mask =
      cv::Mat::zeros(filled.rows + 2, filled.cols + 2, CV_8UC1);
  ...
  cv::Mat opened;
  cv::morphologyEx(filled, opened, cv::MORPH_OPEN, se_disk, cv::Point(-1, -1),
                   2);
...
// ─────────────────────────────────────────────────────────────────────────────
cv::Mat Ccs2DetectorNode::extractSocketShape(const cv::Mat &binary_mask,
...
bool Ccs2DetectorNode::extractSocket(
    const cv::Mat &binary_mask, const cv::Mat &socket_shape,
    const cv::Mat &depth32f, ccs2_socket_detector::msg::SocketDetection &out,
    cv::RotatedRect &rotated_rect, cv::Rect &bbox) {
  ...
  for (size_t i = 0; i < contours.size(); ++i) {
    ...
    double ratio =
        static_cast<double>(bb.height) / static_cast<double>(bb.width);
    ...
    if (use_aspect_filter_) {
      if (ratio < aspect_ratio_min_ || ratio > aspect_ratio_max_) {
        RCLCPP_DEBUG(this->get_logger(),
                     "Contour %zu rejected: ratio=%.2f out of [%.2f, %.2f]", i,
                     ratio, aspect_ratio_min_, aspect_ratio_max_);
        continue;
      }
    }
  ...
  if (best < 0) {
    RCLCPP_DEBUG(this->get_logger(),
                 "No contour passed aspect+area filter (min_area=%.0f).",
                 eff_min_area);
    return false;
  }
  ...
  for (int row = 0; row < roi_mask.rows; ++row) {
    for (int col = 0; col < roi_mask.cols; ++col) {
      if (roi_mask.at<uint8_t>(row, col) == 0) {
        continue;
      }
  ...
  double ratio_used =
      static_cast<double>(bbox.height) / static_cast<double>(bbox.width);
  RCLCPP_INFO(this->get_logger(),
              "Detected — centroid=(%.0f,%.0f)  bbox=[%d,%d %dx%d]  H/W=%.2f  "
              "depth=%.3fm  scale=%.2f",
              out.centroid_x, out.centroid_y, bbox.x, bbox.y, bbox.width,
              bbox.height, ratio_used, out.depth_m, scale);
...
void Ccs2DetectorNode::computeAngles(
    const cv::Mat &depth32f, const cv::Mat &mask, const cv::Rect &bbox,
  ...
  // φ_YZ (paper eq. 11)
  {
    ...
    if (cu > 0 && cl > 0) {
      double ratio =
          std::clamp((su / cu - sl / cl) / (socket_h_mm_ / 1000.0), -1.0, 1.0);
  ...
  // φ_XZ (paper eq. 12)
  {
    ...
    if (cl > 0 && cr > 0) {
      double ratio =
          std::clamp((sl / cl - sr / cr) / (socket_w_mm_ / 1000.0), -1.0, 1.0);
      out.angle_xz = static_cast<float>(std::asin(ratio) * 180.0 / M_PI);
    }
  }

  RCLCPP_INFO(this->get_logger(), "Angles — XY=%.2f°  YZ=%.2f°  XZ=%.2f°",
...
void Ccs2DetectorNode::publishDebug(
    const cv::Mat &bgr, const cv::Mat &mask, const cv::Mat &socket_shape,
    const ccs2_socket_detector::msg::SocketDetection &det, const cv::Rect &bbox,
  ...
  if (debug_pub_->get_subscription_count() > 0) {
    ...
    if (det.detected) {
      if (bbox.x >= 0 && bbox.y >= 0 && bbox.x + bbox.width <= viz.cols &&
      ...
      snprintf(buf, sizeof(buf), "Mode: %s",
               use_color_filter_ ? "HSV" : "GAMMA");
      put(buf, 0);
      snprintf(buf, sizeof(buf), "Centroid: (%.0f, %.0f)", det.centroid_x,
               det.centroid_y);
      put(buf, 1);
      snprintf(buf, sizeof(buf), "Depth: %.3f m", det.depth_m);
      put(buf, 2);
      snprintf(buf, sizeof(buf), "XY: %.2f deg", det.angle_xy);
      put(buf, 3);
      snprintf(buf, sizeof(buf), "YZ: %.2f deg", det.angle_yz);
      put(buf, 4);
      snprintf(buf, sizeof(buf), "XZ: %.2f deg", det.angle_xz);
      put(buf, 5);
      snprintf(buf, sizeof(buf), "Scale: %.2f", depthScale());
      put(buf, 6);
    } else {
...
// makeTile
//
// Produces a BGR tile of exactly `tile_size` pixels:
//   1. Convert single-channel images to BGR.
//   2. Resize to tile_size (keeping the aspect is a nice-to-have; we use
//      simple resize so every tile is identical in size for clean grid concat).
//   3. Draw a filled dark banner at the top, then write the title in white.
// ─────────────────────────────────────────────────────────────────────────────
cv::Mat Ccs2DetectorNode::makeTile(const std::string &title, const cv::Mat &img,
                                   cv::Size tile_size) {
  if (img.empty()) {
    // Return a dark placeholder with the title so the grid still works.
    cv::Mat blank(tile_size, CV_8UC3, cv::Scalar(30, 30, 30));
    cv::putText(blank, "(empty)", cv::Point(8, tile_size.height / 2),
                cv::FONT_HERSHEY_SIMPLEX, 0.55, cv::Scalar(120, 120, 120), 1);
    cv::putText(blank, title, cv::Point(8, 20), cv::FONT_HERSHEY_SIMPLEX, 0.55,
                cv::Scalar(255, 255, 255), 1);
    return blank;
  }
  ...
  // Resize to tile_size
  cv::Mat tile;
  cv::resize(bgr, tile, tile_size);

  // Draw title banner (dark semi-opaque strip at the top)
  const int banner_h = 26;
  cv::Mat banner_roi = tile(cv::Rect(0, 0, tile.cols, banner_h));
  cv::Mat dark(banner_roi.size(), CV_8UC3, cv::Scalar(20, 20, 20));
  cv::addWeighted(dark, 0.65, banner_roi, 0.35, 0, banner_roi);

  // White title text
  cv::putText(tile, title, cv::Point(6, banner_h - 6), cv::FONT_HERSHEY_SIMPLEX,
...
// showPipeline
//
// Assembles 9 tiles into a clean 3×3 grid for paper presentation:
//
//   | RGB          | Gray      | CLAHE         |
//   | Gamma        | Otsu      | Morph Cleanup |
//   | Socket Shape | Detection | Circle Det.   |
//
// circle_img: latest frame from /circle_detector/debug_image; if empty, a
// placeholder is shown instead.
// ─────────────────────────────────────────────────────────────────────────────
void Ccs2DetectorNode::showPipeline(const cv::Mat &rgb_bgr,
                                    const PipelineImages &imgs,
                                    const cv::Mat &circle_img) {
  const cv::Size TILE(320, 240);

  // ── Row 0: input and pre-enhancement ─────────────────────────────────────
  cv::Mat t0 = makeTile("RGB", rgb_bgr, TILE);
  cv::Mat t1 = makeTile("Gray", imgs.gray, TILE);
  cv::Mat t2 = makeTile("CLAHE", imgs.clahe, TILE);

  // ── Row 1: intensity transform and segmentation ───────────────────────────
  cv::Mat t3 = makeTile("Gamma", imgs.gamma, TILE);
  cv::Mat t4 = makeTile("Otsu Binary", imgs.otsu, TILE);
  cv::Mat t5 = makeTile("Morph Cleanup", imgs.morph, TILE);

  // ── Row 2: shape, result, external ───────────────────────────────────────
  cv::Mat t6 = makeTile("Socket Shape", imgs.socket, TILE);
  cv::Mat t7 = makeTile("Final Detection", imgs.detection, TILE);

  // Circle-detector tile: use received image or a dark placeholder
  cv::Mat t8;
  if (circle_img.empty()) {
    cv::Mat ph(TILE, CV_8UC3, cv::Scalar(20, 20, 20));
    cv::putText(ph, "Waiting for", cv::Point(8, TILE.height / 2 - 14),
                cv::FONT_HERSHEY_SIMPLEX, 0.5, cv::Scalar(120, 120, 120), 1,
                cv::LINE_AA);
    cv::putText(ph, "Circle Detector...", cv::Point(8, TILE.height / 2 + 8),
                cv::FONT_HERSHEY_SIMPLEX, 0.5, cv::Scalar(120, 120, 120), 1,
                cv::LINE_AA);
    // still draw the title banner via a dummy tile
    t8 = makeTile("Pose Detector", ph, TILE);
  } else {
    t8 = makeTile("Pose Detector", circle_img, TILE);
  }

  // ── Concatenate into 3×3 dashboard ───────────────────────────────────────
  cv::Mat row0, row1, row2;
  cv::hconcat(std::vector<cv::Mat>{t0, t1, t2}, row0);
  cv::hconcat(std::vector<cv::Mat>{t3, t4, t5}, row1);
  cv::hconcat(std::vector<cv::Mat>{t6, t7, t8}, row2);

  cv::Mat dashboard;
  cv::vconcat(std::vector<cv::Mat>{row0, row1, row2}, dashboard);

  cv::imshow("CCS2 Detection Pipeline", dashboard);
}
...
int main(int argc, char **argv) {
  ...
  rclcpp::spin(std::make_shared<ccs2_socket_detector::Ccs2DetectorNode>());
  rclcpp::shutdown();
  return 0;
}
