...
public:
  ...
  AutoDockingNode() : Node("auto_docking_node") {
    ...
    stations_["1"] = {{7.5, 0.0, 0.0}, {8.5, -0.3, (PI / 2) + 1.5}};
    stations_["2"] = {{7.5, 4.0, 0.0}, {8.5, 3.7, PI / 2}};
    stations_["3"] = {{0.0, 7.5, PI / 2}, {0.3, 8.5, PI}};

    timer_ = create_wall_timer(std::chrono::milliseconds(100),
                               std::bind(&AutoDockingNode::control_loop, this));
...
private:
  void start_mission(const Station &station, const std::string &station_id) {
    ...
    RCLCPP_INFO(this->get_logger(),
                "Đang gửi lệnh tới Nav2: Đi đến điểm chờ Trạm %s...",
                station_id.c_str());
    ...
    send_goal_options.goal_response_callback =
        std::bind(&AutoDockingNode::nav2_goal_response_callback, this,
                  std::placeholders::_1);
    send_goal_options.result_callback = std::bind(
        &AutoDockingNode::nav2_result_callback, this, std::placeholders::_1);
    nav_client_->async_send_goal(goal_msg, send_goal_options);
  }

  void
  nav2_goal_response_callback(const GoalHandleNav2::SharedPtr &goal_handle) {
    if (!goal_handle) {
      RCLCPP_ERROR(this->get_logger(), "Nav2 TỪ CHỐI lệnh!");
      state_ = "IDLE";
    } else {
      RCLCPP_INFO(this->get_logger(),
                  "Nav2 CHẤP NHẬN lệnh. Bắt đầu di chuyển...");
    }
  }
  ...
  void nav2_result_callback(const GoalHandleNav2::WrappedResult &result) {
    if (result.code == rclcpp_action::ResultCode::SUCCEEDED) {
      RCLCPP_INFO(this->get_logger(),
                  "Tới điểm chờ thành công! Bắt đầu trượt lùi chuồng...");
      state_ = "PID";
      pid_step_ = 0;
      sum_err_x_ = 0.0;
      sum_err_y_ = 0.0;
  ...
  void control_loop() {
    ...
    try {
      trans = tf_buffer_->lookupTransform("map", "base_footprint",
                                          tf2::TimePointZero);
    ...
    double local_dx =
        std::cos(curr_yaw) * dx_global + std::sin(curr_yaw) * dy_global;
    double local_dy =
        -std::sin(curr_yaw) * dx_global + std::cos(curr_yaw) * dy_global;
